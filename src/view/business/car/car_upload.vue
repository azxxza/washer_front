<template>
  <Modal v-model="isShow" @on-cancel="handleClose" title="设备信息导入" width="68">
    <div>
      <h4>导入规则：</h4>
...
<script>
import { Modal,Row,Col,Form,FormItem,Upload } from 'view-design' 
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        { title: "sn", key: "sn" },
        { title: "mac", key: "mac" },
        { title: "primaryKey", key: "primaryKey" }
      ]
    };
  },
  methods: {
    ...
    handleDownload() {
      const params = {
        title: ["sn", "mac", "primaryKey"],
        key: ["sn", "mac", "primaryKey"],
        data: [],
        autoWidth: true,
        filename: "设备模版下载"
      };
  ...
  computed: {
    uploadUrl() {
      return baseURL + "api/careCar/uploadExcel";
    }
  }
};
</script>
