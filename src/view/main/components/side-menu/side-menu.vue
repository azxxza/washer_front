  ...
  <div class="side-menu-wrapper">
    ...
    <Menu ref="menu" v-show="!collapsed" :active-name="activeName" :open-names="openedNames" :accordion="accordion" :theme="theme" width="auto" @on-select="handleSelect">
      <template v-for="item in menuList">
        ...
        <template v-if="item.children && item.children.length === 1">
          <side-menu-item v-if="showOneChildren(item)" :key="`menu-${item.name}`" :parent-item="item"></side-menu-item>
    ...
    <div class="menu-collapsed" v-show="collapsed" :list="menuList">
      <template v-for="item in menuList">
        ...
        <Tooltip transfer v-else :content="(item.title) || (item.children && item.children[0] && item.children[0].title)" placement="right" :key="`drop-menu-${item.title}`">
